            ...
            <div className="product-item bg-light mb-4">
                <div className="product-img position-relative overflow-hidden">
                    {product.images.slice(0,1).map(imgData => (
                        <img className="img-fluid w-100" src={imgData.src} alt={imgData.name} key={imgData.id}/>
                    ))}
                ...
                <div className="text-center py-4">
                    ...
                    <div className="d-flex align-items-center justify-content-center mt-2">
                        <h5>${product.prices.price}</h5><h6 className="text-muted ml-2"><del>${product.prices.regular_price}</del></h6>
                    </div>
...
export const getServerSideProps = async (context) => {
    ...
    const res = await axios.get(`https://wpfurniture.mangoitsol.com/wp-json/wc/store/products?category=${params.id}`);
